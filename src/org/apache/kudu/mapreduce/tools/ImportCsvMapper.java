...
//        	row.setNull(colName);
//        }
        if(col.getType() == Type.INT8) {
        	if (colValue == "") {
				row.setNull(colName);
			} else {
				row.addByte(colName, Byte.parseByte(colValue));
			}
		} else if(col.getType() == Type.INT16) {
			if (colValue == "") {
				row.setNull(colName);
			} else {
				row.addShort(colName, Short.parseShort(colValue));
			}
		} else if(col.getType() == Type.INT32) {
			if (colValue == "") {
				row.setNull(colName);
			} else {
				row.addInt(colName, Integer.parseInt(colValue));
			}
		} else if(col.getType() == Type.INT64) {
			if (colValue == "") {
				row.setNull(colName);
			} else {
				row.addLong(colName, Long.parseLong(colValue));
			}
		} else if(col.getType() == Type.STRING) {
			if (colValue == "") {
				row.setNull(colName);
			} else {
				row.addString(colName, colValue);
			}
		} else if(col.getType() == Type.FLOAT) {
			if (colValue == "") {
				row.setNull(colName);
			} else {
				row.addFloat(colName, Float.parseFloat(colValue));
			}
		} else if(col.getType() == Type.DOUBLE) {
			if (colValue == "") {
				row.setNull(colName);
			} else {
				row.addDouble(colName, Double.parseDouble(colValue));
			}
		} else if (col.getType() == Type.BOOL ) {
			if (colValue == "") {
				row.setNull(colName);
			} else {
				row.addBoolean(colName, Boolean.parseBoolean(colValue));
